<template>
  <div class="achievement-system">
    <div class="achievement-header">
      ...
      <span class="header-title">妹妹成就</span>
...
const props = defineProps<{
  brotherComplex: number; // 兄控度
  mood: number; // 心情
  pocketMoney: number; // 零花钱
}>();
...
const achievements = ref<Achievement[]>([
  ...
  },
  {
    ...
    unlocked: false,
    progress: 0,
    condition: () => props.brotherComplex >= 30,
  },
  {
    id: 'best_brother',
    name: '最好的哥哥',
    ...
    unlocked: false,
    progress: 0,
    condition: () => props.brotherComplex >= 50,
  },
  {
    id: 'possessive',
    name: '独占欲',
    ...
    unlocked: false,
    progress: 0,
    condition: () => props.brotherComplex >= 70,
  },
  {
    id: 'secret_touch',
    name: '秘密接触',
    description: '进行秘密的亲密接触',
    icon: '🤫',
    unlocked: false,
    secret: true,
    condition: () => props.brotherComplex >= 40,
  },
  {
    id: 'max_complex',
    name: '无法离开',
    description: '兄控度达到100',
    icon: '❤️',
    unlocked: false,
    progress: 0,
    condition: () => props.brotherComplex >= 100,
  },
  {
    id: 'happy_girl',
    name: '幸福满满',
    description: '心情保持在90以上',
    icon: '😊',
    unlocked: false,
    condition: () => props.mood >= 90,
  },
]);
...
function updateAchievementProgress() {
  achievements.value.forEach(achievement => {
    ...
    // 计算进度
    if (achievement.id === 'good_brother') {
      achievement.progress = Math.min(100, Math.round((props.brotherComplex / 30) * 100));
    } else if (achievement.id === 'best_brother') {
      achievement.progress = Math.min(100, Math.round((props.brotherComplex / 50) * 100));
    } else if (achievement.id === 'possessive') {
      achievement.progress = Math.min(100, Math.round((props.brotherComplex / 70) * 100));
    } else if (achievement.id === 'max_complex') {
      achievement.progress = props.brotherComplex;
    } else if (achievement.id === 'happy_girl') {
      achievement.progress = Math.min(100, Math.round((props.mood / 90) * 100));
    }
...
// 监听属性变化
watch(
  [() => props.brotherComplex, () => props.mood, () => props.pocketMoney],
  () => {
